...
| First-party caller | Authenticated code, not automatically authorized | Identified with `GetInvokingResource`; allowed operation and reason are validated |
...
- catalog defines stack, weight, use and metadata policy;
- scalar-only metadata allowlist with key/value and serialized-size limits;
...
- usable handlers are registered server-side by identified resources;
...
- events use metadata allowlists;
