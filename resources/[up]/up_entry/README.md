...
Client synchronization is event-driven. Resource restart triggers a new `clientReady` handshake, but a client that retains replicated `up:entry = true` while missing the corresponding `entered` event has no watchdog yet. Add state-divergence recovery before public release if runtime evidence shows this window can occur; the handler must remain idempotent and must never create a server session.
